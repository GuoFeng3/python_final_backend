...
from house_data.views import direction_price
...
def test_view():
    ...
    request = factory.get('/house_data/direction-price/', {'行政区名': '昌平'})
    response = direction_price(request)
    ...
    request_all = factory.get('/house_data/direction-price/', {'行政区名': 'all'})
    response_all = direction_price(request_all)
